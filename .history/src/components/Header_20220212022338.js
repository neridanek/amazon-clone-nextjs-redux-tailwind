...
const Header = () => {
  ...
  return (
    <div
      className="flex items-center bg-blue-900 py-3 text-white text-sm
    sm:px-5 "
    >
      <Image
        onClick={() => router.push('/')}
      ...
      />
      <div className="flex flex-1 items-center px-10 sm:px-15">
      ...
      <div onClick={!session ? signIn : signOut} className="link px-5 sm:px-3 ">
        <h1>{session ? `Hello,${session.user.name}` : 'Sign In'}</h1>
        <p className="font-bold">Konto i listy</p>
      </div>
      <div onClick={() => router.push('/orders')} className="link px-5 sm:px-3">
        <h1>Zwroty</h1>
      ...
      >
        <span className="absolute top-0 right-14 bg-yellow-400 h-5 w-5 text-center rounded-full text-black font-bold sm:top-2">
          {items.length}
        </span>
