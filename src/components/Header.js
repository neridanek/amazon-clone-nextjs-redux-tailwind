...
const Header = () => {
    ...
    return (
        <div className="flex items-center bg-amazon_blue py-3 text-white text-sm ">
            <Image
                onClick={()=>router.push("/")}
            ...
            <SearchIcon className="text-black bg-yellow-400 rounded-r p-3 h-10"/>
            </div>
            ...
            <div className="link px-5">
                <h1>Zwroty</h1>
                <p className="font-bold">i zamówienia</p>
            </div>
            <div onClick={()=>router.push("/checkout")} className="flex items-center px-5">
                <span className="absolute top-0 right-14 bg-yellow-400 h-5 w-5 text-center rounded-full text-black font-bold">{items.length}</span>
